...
export default class Chat extends Component {
    ...
    getmessagedata = async () => {
        ...
        var inbox_id_other = 'u_' + userdata.user_id;
        ...
        if (FirebaseInboxJson.length > 0) {
            var find_inbox_index2 = FirebaseInboxJson.findIndex(x => x.user_id == other_user_id);
    ...
    sendmessagecallbtn = async (messageType, message) => {
       ...
       setTimeout(() => {
        ...
        query.once('value', (data) => {
            ...
            consolepro.consolelog('blockinbox', blockinbox);
            if (blockinbox == 'no') {
                firebaseprovider.UpdateUserInboxOther(other_user_id_send, inbox_id_other, jsonUserDataOther);
            }
                                ...
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    ...
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        ...
                                        <View style={{
                                            ...
                                            {this.state.blocked_by_other_user == 'no' &&
                                            <>
                                ...
                                renderItem={({ item, index }) => {
                                    return (
                                        <View style={{ width: '97%', alignSelf: 'center', paddingVertical: 7 }}>
                                            {item.userid != this.state.user_id && <View style={{ alignSelf: 'flex-start', width: '75%' }}>
                                                <View style={{ backgroundColor: Colors.theme_color, paddingVertical: 1.5, paddingHorizontal: 8, alignSelf: 'flex-start', borderTopStartRadius: 6, borderTopEndRadius: 6, borderBottomLeftRadius: 6 }}>
                                                    {item.messageType == 'text' && <Text style={{ fontSize: mobileW * 5/100, fontFamily: Font.FontBold, color: Colors.whiteColor }}>{item.name}</Text>}
                                            ...
                                            {item.userid == this.state.user_id && <View style={{ width: '75%', alignSelf: 'flex-end', alignItems: 'flex-end', }}>
                                                <View style={{ backgroundColor: Colors.theme_color, borderTopStartRadius: 6, borderTopEndRadius: 6, borderBottomLeftRadius: 6, paddingVertical: 1.5, paddingHorizontal: 8, alignSelf: 'flex-end' }}>
                                                    {item.messageType == 'text' && <Text style={{ fontSize: mobileW * 5/100, fontFamily: Font.FontMedium, color: Colors.whiteColor }}>{item.name}</Text>}
                                                    {item.messageType == 'image' && <TouchableOpacity activeOpacity={0.9} onPress={() => { this.props.navigation.navigate('ViewImage', { 'image': item.name }) }}>
                                ...
                                <TouchableOpacity
                                    ...
                                    <View style={{ width: '100%', marginLeft: mobileW * 2 / 100 }}>
                                        <TextInput
                                            ...
                                            onFocus={() => { this.setState({ bottom: this.state.notch == true ? 45 : 22 }); }}
                                            onBlur={() => { this.setState({ bottom: 0 }) }}
                                        >
                                        </TextInput>
                                    </View>
                                </TouchableOpacity>
